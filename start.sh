...
cat /kafka/config/server.properties.template | sed \
  ...
  -e "s|{{OFFSETS_RETENTION_MINUTES}}|${OFFSETS_RETENTION_MINUTES:-1440}|g" \
  ...
  -e "s|{{LOG_MESSAGE_TIMESTAMP_TYPE}}|${LOG_MESSAGE_TIMESTAMP_TYPE:-CreateTime}|g" \
  -e "s|{{LOG_CLEANER_MIN_COMPACTION_MS}}|${LOG_CLEANER_MIN_COMPACTION_MS:-3600000}|g" \
